 ...
 */
inline fun <T> Flow<T>.launchSafe(
...
) = coroutineScope.launch { collect { action(it) } }

inline fun <T> Flow<T>.collectOn(
    coroutineContext: CoroutineContext,
    crossinline action: suspend (value: T) -> Unit,
) = CoroutineScope(coroutineContext).launch { collect { action(it) } }

inline fun <T> FlowCollector<T>.emitOn(
    coroutineScope: CoroutineScope,
    crossinline value: () -> T,
) = coroutineScope.launch { emit(value()) }

inline fun <T> FlowCollector<T>.emitOn(
    coroutineContext: CoroutineContext,
    crossinline value: () -> T,
) = CoroutineScope(coroutineContext).launch { emit(value()) }

/**
 * The terminal operator that returns the first element emitted by the flow matching the given [predicate] mapped to [V] and then cancels flow's collection.
 * Returns [null] if the flow did not contain an element matching the [predicate].
 *
 * @see firstOrNull
 */
suspend fun <T, V> Flow<T>.firstOfOrNull(predicate: suspend (T) -> V?): V? {
    var result: V? = null
